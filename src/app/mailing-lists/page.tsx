...
import { dalGetMailingListSubscribers, dalGetMailingLists } from "../../dal/mailing_lists";
...
export default function MailingListsPage(props: PageProps<"/mailing-lists">) {
	...
	const selectedListNamePromise = searchParamsPromise.then((p) => parseParams(p.listName));
	const searchPromise = searchParamsPromise.then((p) => parseParams(p.client) || parseParams(p.search));

	const listsResPromise = dalGetMailingLists();
	
	const activeListPromise = Promise.all([listsResPromise, selectedListNamePromise]).then(([listsRes, listName]) => {
	...
	const subscribersResPromise = activeListPromise.then(activeList => {
		if (!activeList) return { ok: true, value: [] };
