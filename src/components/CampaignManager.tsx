...
interface CampaignManagerProps {
	campaignsPromise: Promise<{ ok: boolean; value?: Campaign[]; error?: any }>;
	mailingListsPromise: Promise<{ ok: boolean; value?: MailingList[]; error?: any }>;
...
export default function CampaignManager({
	...
	const campaigns = campaignsRes.ok && campaignsRes.value ? campaignsRes.value : [];
	const dbError = !campaignsRes.ok && campaignsRes.error ? campaignsRes.error.message : null;
	...
	const filteredCampaigns = campaigns.filter(c => 
        ...
        c.mailing_list_name?.toLowerCase().includes(currentSearch.toLowerCase()) ||
		...
		<div className="flex flex-col gap-6">
			...
			{dbError && (
				<div className="p-4 rounded-xl border border-yellow-250 bg-yellow-50/70 text-yellow-900 text-xs">
					...
					Failed to connect to Neon DB: {dbError}. Using simulated campaign databases.
				</div>
			)}
